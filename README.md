# Common Lisp replit using sbcl

> WARNIGN: This replit is my personal experimentation room with lisp on replit.  
> Do not rely on this replit to stay consistent in the future!

## Tips and tricks with sbcl

### In the interactive repl
1. use `rlwrap sbcl` to wrap the default sbcl repl into readline capabilities (for example to use the arrow keys).
2. `*`, `**` and `***` are the last, second last, and third last values the repl calculated.  
   `+`, `++` and `+++` are the last, second last, and third last forms the repl evaluated.  
   That means you can use the stars if you want the return values and the pluses when you want what you inputted.
3. `-` is the current form being evaluated in the repl.
